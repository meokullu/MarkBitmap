    ...
    public partial class MarkBitmap
    {
        ...
        public static byte[] MarkHorizontally(byte[] buffer, int count, int width, int height, Color color)
        {
            ...
            for (int i = 1; i < count; i++)
            {
                ...
                buffer = MarkHorizontalLine(buffer, x: offset * i, y: 0, width: width, length: width, color: color);
            }
        ...
        public static byte[] MarkVerticalLine(byte[] buffer, int x, int y, int width, int length, Color color)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                buffer[(y + (i * width)) * 3] = color.B;
                buffer[(y + (i * width)) * 3 + 1] = color.G;
                buffer[(y + (i * width)) * 3 + 2] = color.R;
            }
        ...
        {
            // Top left
            buffer = MarkHorizontalLine(buffer, 0, 0, width, length, color);
            buffer = MarkVerticalLine(buffer, 0, 0, width, length, color);

            // Bottom left
            buffer = MarkHorizontalLine(buffer, 0, height - 1, width, length, color);
            buffer = MarkVerticalLine(buffer, 0, height - 1 - length, width, length, color);

            // Top right
            buffer = MarkHorizontalLine(buffer, width - 1 - length, 0, width, length, color);
            buffer = MarkVerticalLine(buffer, width - 1, 0, width, length, color);

            // Top bottom
            buffer = MarkHorizontalLine(buffer, width - 1 - length, height - 1, width, length, color);
            buffer = MarkVerticalLine(buffer, width - 1, height - 1 - length, width, length, color);
